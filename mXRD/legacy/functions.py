...
class TIFF:
    def __init__(self,filename=None,
        ...
        if not filename:
            if pos1 and pos2:
                self.filename = '{:s}_{:s}_pos1_{:.3f}mm_pos2_{:.3f}mm_{:s}_{:04d}'.\
                    format(sample_name,date_time,pos1,pos2,meta_id,ind)
            elif pos1 and not pos2:
                self.filename = '{:s}_{:s}_pos1_{:.3f}mm_{:s}_{:04d}'.\
                    format(sample_name,date_time,pos1,meta_id,ind)
            elif not pos1 and pos2:
                self.filename = '{:s}_{:s}_pos2_{:.3f}mm_{:s}_{:04d}'.\
                    format(sample_name,date_time,pos2,meta_id,ind)
            else:
    ...
    def read(self,meta_id=None,tiff_ext='.tiff',read_2d=False,load_2d=False):         
        ...
        for i in tiffs:
            ...
            if i.split('_')[-2] == 'corrected':
                pos1_pos2_positions = i[0:-24].split(date_time)[1].split('_')[1:-2]
                meta_id = i.split('_')[-5]
                ind = int(i.split('_')[-4])
            else:
                pos1_pos2_positions = i[0:-5].split(date_time)[1].split('_')[1:-2]   
                meta_id = i.split('_')[-2]
                ind = int(i.split('_')[-1])

            if pos1_pos2_positions:
                if len(pos1_pos2_positions) == 4:
                    pos1 = float(pos1_pos2_positions[1].split('mm')[0]) 
                    pos2 = float(pos1_pos2_positions[3].split('mm')[0])
                elif pos1_pos2_positions[0] == 'pos1':
                    pos1 = float(pos1_pos2_positions[1].split('mm')[0])
                    pos2 = None
                elif pos1_pos2_positions[0] == 'pos2':
                    pos2 = float(pos1_pos2_positions[1].split('mm')[0])
                    pos1 = None 
            else:
                pos1 = None
                pos2 = None
    ...
    def plot(self,plot_type=None,q_range=(0.5,7.5),q_range_metric=(1,6),
        ...
        if plot_type=='2d-scan': 
            ...
            for c,i in enumerate(self.tiffs):
                ...
                pos1pos2s.append([i.pos1,i.pos2])
                if numbers_on:
                    ax.text(text_xyshift+i.pos1,text_xyshift+i.pos2,str(c),fontsize=10,alpha=0.7)
                
            pos1pos2s = np.array(pos1pos2s)
            metrics = np.array(metrics)
            metrics = metrics-np.mean(metrics)
            ax.scatter(pos1pos2s[:,0],pos1pos2s[:,1],c=metrics,cmap='jet',s=200)  
            ax.set_xlabel('pos1 (mm)', fontsize=14, weight='bold')
            ax.set_ylabel('pos2 (mm)', fontsize=14, weight='bold')             
            ...
            ax.set_xlabel(u'Q ($\AA^{-1}$)', fontsize=14, weight='bold')
            ax.set_ylabel(u'Intensity', fontsize=14, weight='bold') 
            pos1s = np.array(pos1s); pos2s = np.array(pos2s) 
            ...
            if pos1s.any():
                ...
                if numbers_on:
                    for i in indexes:
                        ax.text(self.tiffs[i].pos1,max_intensities[i]*1.02,str(i),fontsize=10,weight='bold')          
            elif pos2s.any():
                ax.bar(pos2s,height=max_intensities,width=0.2,color=mcolors)  
                ax.set_xlabel('pos2 (mm)', fontsize=14, weight='bold') 
                if numbers_on:
                    for i in indexes:
                        ax.text(self.tiffs[i].pos2,max_intensities[i]*1.02,str(i),fontsize=10,weight='bold')                  
            ax.set_yscale('log')
...
def center_finder(smpl,meta_id=None,eps=0.05, min_samples=None, n_jobs=1,
    ...
    if plot:
        ...
        if isinstance(center,np.ndarray):
            ...
            ax1.set_xlabel('pos1 (mm)', fontsize=10, weight='bold')
            ax1.set_ylabel('pos2 (mm)', fontsize=10, weight='bold')  
        
            ax1.set_title('{:s} ({:s}) center @ \'pos1={:5.2f}mm,pos2={:5.2f}mm\''
