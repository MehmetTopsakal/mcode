...
def readVASPRUNXML(file):
    ...
    params.append(NSPIN)
    params.append(NKPTS)
    params.append(NBANDS)
    params.append(NEDOS)    
    params.append(NATOMS)
    params.append(NTYPES)
    params.append(NELECT)
    params.append(NSW)    
    params.append(ISIF)     
...
# re-arrange pdos
    ...
    for k in range(len(klines_new)-1):
        ...
        k1=klines_new[k+1]
        dx=k0[0]-k1[0]; dy=k0[1]-k1[1]; dz=k0[2]-k1[2]
        newk=klines_path[k]+sqrt(dx*dx+dy*dy+dz*dz)
...
## bandgap part 
    ...
    params.extend(gap_data)

    Edos  = np.array(Edos, np.float) 
    tdos_up  = np.array(tdos_up, np.float) 
    tdos_dw  = np.array(tdos_dw, np.float)
    pdos_up  = np.array(pdos_up, np.float) 
    pdos_dw  = np.array(pdos_dw, np.float)     
    kpointsDir  = np.array(kpointsDir, np.float)
    params  = np.array(params, np.float)



    poscar0 = np.array(poscar0, np.float) 
    return params, kpointsDir, kpoints_path, klines_path, poscar0, bands_up, bands_dw, efermi, Edos, tdos_up, tdos_dw, pdos_up, pdos_dw


params, kpointsDir, kpoints_path, klines_path, poscar0, bands_up, bands_dw, efermi, Edos, tdos_up, tdos_dw, pdos_up, pdos_dw = readVASPRUNXML('vasprun.xml')
