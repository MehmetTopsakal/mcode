 function [minimum,fit,res] = mfitx(file,column_x,column_y,order,format,lower_boundary,upper_boundary)
 %
 % usage 
 %
...
xi=[xmin:(xmax-xmin)/1000:xmax] ; yi = polyval(p,xi); plot(xi,yi,'-'); forsave = [ xi' yi'] ; save -ascii forsave.dat forsave
ex = abs((max(eff_data(:,1)) - min(eff_data(:,1)))/20) ; 
ey = abs((max(eff_data(:,2)) - min(eff_data(:,2)))/20) ; 
xlim([ (min(eff_data(:,1))-ex)  (max(eff_data(:,1))+ex) ]); 
ylim([ (min(eff_data(:,2))-ey)  (max(eff_data(:,2))+ey) ]);
