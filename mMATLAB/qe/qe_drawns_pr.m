function qe_drawns_pr(spin,file,iatom,iwrite,azi,ele,dmax,isovalue,customsum)
...
orb2 = -sqrt(3/2)*0.5*xx.*(5*zz.^2-1); 
orb3 = -sqrt(3/2)*0.5*yy.*(5*zz.^2-1); 
orb4 = 0.5*sqrt(15)*zz.*(xx.^2-yy.^2);
orb5 = sqrt(15)*xx.*yy.*zz;
orb6 = -0.5*sqrt(5/2)*xx.*(xx.^2-3*yy.^2);      
orb7 = -0.5*sqrt(5/2)*yy.*(3*xx.^2-yy.^2);      
