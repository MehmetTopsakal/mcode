...
def plot_smpl(smpl,meta_id=None,q_range=(0.5,7.5),q_range_metric=(1,6),
    ...
    linescan_tiffs = []
    for i in smpl.tiffs:
        if i.info['dx'] and not i.info['dy']:
            if i.info['meta_id'] == meta_id:
                linescan_tiffs.append([i,i.info['ind']])            
        elif i.info['dy'] and not i.info['dx']:
            if i.info['meta_id'] == meta_id:
                linescan_tiffs.append([i,i.info['ind']])             
    ...
    if gridscan_tiffs:
        ...
        for c,i in enumerate(gridscan_tiffs):
            ...
            dxdys.append([i.info['dx'],i.info['dy']])
            if numbers_on:
                ax.text(text_xyshift+i.info['dx'],text_xyshift+i.info['dy'],str(c),color='y',fontsize=5,alpha=0.9)

        dxdys = np.array(dxdys)
        metrics = np.array(metrics)
        metrics = metrics-np.mean(metrics)
        ax.scatter(dxdys[:,0],dxdys[:,1],marker='+',s=20)  
        ax.scatter(dxdys[:,0],dxdys[:,1],c=metrics,marker='s',cmap='jet',s=markersize,alpha=0.7)          
        ax.set_xlabel('pos_x (mm)', fontsize=14, weight='bold')
        ax.set_ylabel('pos_y (mm)', fontsize=14, weight='bold') 
        ...
        for e,i in enumerate(linescan_tiffs):
            ...
            indexes.append(e)
            if i.info['dx']:
                dxs.append(i.info['dx'])
            else:
                dys.append(i.info['dx'])
        ax.set_xlim(q_range[0],q_range[1])    
        ax.set_ylim(bottom=max(y_bottom,min(min_intensities)))
        ...
        if numbers_on:
            for i in indexes:
                ax.text(linescan_tiffs[i].info['dx']+text_xyshift,max_intensities[i]*1.02,str(i),fontsize=10,weight='bold')         
