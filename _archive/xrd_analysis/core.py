...
class TIFF():
        ...
        try:
            ...
            if filename.endswith('_dark_corrected_img'):
                ...
                if filename.split('_')[-6] == datetime:
                    ...
                    if filename.split(datetime)[1].startswith('_xfine_'):
                        dx = float(filename.split(datetime)[1].split('_xfine_')[1].split('mm')[0].replace(',','.')[0:-2])
                        try:
                            dy = float(filename.split(datetime)[1].split('_yfine_')[1].split('mm')[0].replace(',','.')[0:-2])
                        except:
                            dy = None                    
                ...
                #yukardakini de ayarla
                if filename.split('_')[-4] == 'temp':  
                    temp = float(filename.split('_')[-3][:-1])
                else:
                    temp = []
                
                if filename.split('_')[-3] == datetime:
                    dx = None
                    dy = None
                elif filename.split('_')[-6] == datetime:
                    dx = None
                    dy = None
                else:
                    if filename.split(datetime)[1].startswith('_dx_'):
                        dx = float(filename.split(datetime)[1].split('_dx_')[1].split('mm')[0].replace(',','.'))
                        try:
                            dy = float(filename.split(datetime)[1].split('_dy_')[1].split('mm')[0].replace(',','.'))
                        except:
                            dy = None
                    else:
                        dx = None
    ...
    def plot(self,y_bottom=2,savefig=False):   
        ...
        def plotter(cmap,vmax,vmin):
            ...
            if masked_img_avail:
                ax.set_title(self.info['filename']+' (masked)')
            else:
                ax.set_title(self.info['filename']) 
