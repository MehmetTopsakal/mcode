function [copt,sopt,sdopt,ropt,areaopt,rtopt]=als_t4iss(nit,tolsigma,inorm,isp);
...
load efa.mat xbackward ebl xforward efl   d e eforward ebackward d Energy;
x0 = e;
...
if nargin<1,nit=300;end
if isempty(nit) | nit==0,nit=300;end

if nargin<2,tolsigma=0.1;end
if isempty(tolsigma) | tolsigma==0,tolsigma=0.1;end
    
if nargin<3,inorm=1;end    
if isempty(inorm) | inorm>2,inorm=1;end
    
if nargin<4 & nexp==1,isp=ones(1,nsign);end
if isempty(isp) | isp==0, isp=ones(1,nsign);end
    
 
if nargin<5,csel=[];end
if nargin<6,ssel=[];end
if nargin<7,vclos1=0;end
if nargin<8,vclos2=0;end
if nargin<6,scons='n';end
if nargin<5,ccons='n';end
if nexp==1,
   ncinic(nexp)=1;
   ncfin(nexp)=ncol;
   nrinic(nexp)=1;
...
wcons = 1 %input('Enter the constraints to be applied in the optimization: ');
...
if ~isempty(c1) 
disp(' ')
ineg= 2 %input('Enter the non-negativity constraints (1=conc / 2=conc and spectra / 3=spectra ): ');
disp(' ')
...
if ineg==3|ineg ==2
		...
		disp(' ')
		ialgs= 2 %input('Enter the selected algorithm for spectra (0 = forced to zero / 1 = nnls / 2 = fnnls)  ');
...
if ineg==1 || ineg ==2
		...
		disp('Constraint of some selected species is only possible with the "forced to zero" option')	
		disp(' ')
		ialg= 2 %input('Enter the selected algorithm for conc (0 = forced to zero / 1 = nnls / 2 = fnnls)  ');
...
if isempty(c3)
	...
	disp(' 			      2 = spectra (rows) equal at tail ')
	inorm= inorm %input('normalization 0, 1 or 2?: ');
...
gr= 'n' %input('Do you want graphic output during the ALS optimization (y/n)? ','s');
	if gr=='Y' || gr =='y'
		gr='y';
	end
...
disp(' ')
answer= 'y' %input('Is all the input information correct (y/n)? ','s');
...
if inorm==3, abss=normv2(abss); end
...
%   subplot(2,1,2);plot(sopt');title('pure spectra in optimal iteration');
   save -7 als.mat copt sopt sdopt ropt areaopt rtopt Energy; exit   
