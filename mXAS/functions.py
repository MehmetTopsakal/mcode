    ...
    if plot_channels:
        ...
        for e, i in enumerate(reads):
            ax.text(
                0,
                e * dy,
                "%s [%d,%.2f,%.2f]" % (i[2][-30:], e, i[3][0][0], i[3][0][-1]),
                color="C%d" % (e % 10),
        ...
        if sdd:
            for i in reads:
                ...
                ax.plot(
                    ...
                    (
                        -(
                            np.array([i[3][s] for s in sdd_cols]).mean(axis=0)
                            # i[3][sdd_cols[0]]
                            # + i[3][sdd_cols[1]]
                            # + i[3][sdd_cols[2]]
                            # + i[3][sdd_cols[3]]
...
def process_ds(
    ...
    if plot_raw:
        ...
        ax_in.plot(group.energy, group.mu, color="k", lw=1)
        ax_in.plot(group.energy, group.pre_edge, lw=0.5)
        ax_in.plot(group.energy, group.post_edge, lw=0.5)
        if glitches is not None or []:
            for g in glitches:
                ax_in.axvline(
                    x=float(g.split(":")[0]),
